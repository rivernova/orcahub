          ...
          <TableBody>
            {images.length === 0 ? (
              <EmptyTableRow
                cols={7}
                icon="🖼"
                title={state.loading ? 'Loading images…' : 'No images'}
                description={state.loading ? undefined : 'Pull an image to get started'}
              />
            ) : (
              images.map(img => (
                <ImageRow key={img.id} image={img} maxSize={maxSize} onDelete={() => deleteImage(img.id)} />
              ))
            )}
          </TableBody>
        </Table>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                placeholder="nginx:latest, postgres:16, ubuntu:22.04"
            ...
            <div className="flex gap-2 flex-wrap">
              {['nginx:latest','redis:alpine','postgres:16','node:20-alpine'].map(s => (
                <button key={s} onClick={() => setPullRef(s)}
...
function ImageRow({ image: img, maxSize, onDelete }: { image: DockerImage; maxSize: number; onDelete: () => void }) {
  ...
  return (
    <TableRow>
      ...
      <TableCell>
        <Badge variant="cyan" className="font-mono">{tag ?? 'latest'}</Badge>
      </TableCell>
      <TableCell>
        <span className="font-mono text-[11.5px]">{shortId(img.id)}</span>
      </TableCell>
      <TableCell>
        <div className="flex items-center gap-2">
          <span className="font-mono text-[11.5px] text-[var(--text-secondary)] w-16">{formatBytes(img.size)}</span>
          <div className="w-20 h-[3px] bg-[var(--bg-raised)] rounded-full overflow-hidden">
            <div className={cn('h-full rounded-full bg-gradient-to-r from-[#7c3aed] to-[#a78bfa]')} style={{ width: `${sizePct}%` }} />
          </div>
        </div>
      </TableCell>
      <TableCell>{formatUptime(img.created)} ago</TableCell>
      <TableCell>
