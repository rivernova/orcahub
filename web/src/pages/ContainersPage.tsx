...
export function ContainersPage() {
  ...
  const filtered = useMemo(() => {
    switch (filter) {
      ...
      case 'stopped': return containers.filter(c => c.state === 'created' || c.state === 'stopped')
      case 'exited':  return containers.filter(c => c.state === 'exited')
      case 'paused':  return containers.filter(c => c.state === 'paused')
    ...
    <div className="animate-pagein">
      ...
      <Card className="overflow-hidden">
        <Table>
          ...
          <TableBody>
            {filtered.length === 0 ? (
              <EmptyTableRow cols={6} icon="📦"
                title={state.loading ? 'Loading…' : 'No containers'}
              ...
              filtered.map(c => (
                <ContainerRow
                  ...
                  onInspect={(id, name) => setInspect({ open: true, id, name })}
                />
              ))
            )}
          </TableBody>
      ...
      <ExecDialog
        open={exec.open} onClose={() => setExec(e => ({ ...e, open: false }))}
        containerId={exec.id} containerName={exec.name}
      />
      <LogsDialog
        open={logs.open} onClose={() => setLogs(l => ({ ...l, open: false }))}
        containerId={logs.id} containerName={logs.name}
      />
      <InspectDialog
        open={inspect.open} onClose={() => setInspect(i => ({ ...i, open: false }))}
        containerId={inspect.id} containerName={inspect.name}
      />
      <DeployDialog
        open={deployOpen} onClose={() => setDeployOpen(false)}
        onDone={loadAll} toast={toast}
      />
    </div>
  )
}
...
function ContainerRow({
  container: c, loading, onAction, onExec, onLogs, onInspect,
}: {
  container: Container
  loading:   boolean
  onAction:  (id: string, a: 'start' | 'stop' | 'restart' | 'pause' | 'unpause' | 'delete') => void
    ...
    <TableRow className={cn(loading && 'opacity-50')}>
      <TableCell>
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded-[7px] bg-[var(--bg-raised)] border border-[var(--border)] flex items-center justify-center text-xs flex-shrink-0">📦</div>
          <div>
      ...
      <TableCell>
        <div className="flex items-center gap-1">
          {isPaused ? (
            <Btn title="Resume"  danger={false} success onClick={() => onAction(c.id, 'unpause')} disabled={loading}><Play className="w-3 h-3" /></Btn>
          ) : isRunning ? (
            <>
              <Btn title="Pause"   onClick={() => onAction(c.id, 'pause')}   disabled={loading}><Pause className="w-3 h-3" /></Btn>
              <Btn title="Restart" onClick={() => onAction(c.id, 'restart')} disabled={loading}><RotateCcw className="w-3 h-3" /></Btn>
              <Btn title="Stop"    onClick={() => onAction(c.id, 'stop')}    disabled={loading} danger><Square className="w-3 h-3" /></Btn>
            </>
          ) : (
            <Btn title="Start" onClick={() => onAction(c.id, 'start')} disabled={loading} success><Play className="w-3 h-3" /></Btn>
          )}
          <Btn title="Inspect" onClick={() => onInspect(c.id, name)}><Search className="w-3 h-3" /></Btn>
          <Btn title="Exec"    onClick={() => onExec(c.id, name)} disabled={!isRunning}><Terminal className="w-3 h-3" /></Btn>
          <Btn title="Logs"    onClick={() => onLogs(c.id, name)}><FileText className="w-3 h-3" /></Btn>
          <Btn title="Delete"  onClick={() => onAction(c.id, 'delete')} disabled={loading} danger><Trash2 className="w-3 h-3" /></Btn>
        </div>
      </TableCell>
    </TableRow>
  )
...
function Btn({ children, title, onClick, disabled = false, danger = false, success = false }: {
  children: React.ReactNode; title: string; onClick: () => void
  disabled?: boolean; danger?: boolean; success?: boolean
}) {
  return (
    <button onClick={onClick} disabled={disabled} title={title}
      className={cn(
        'w-[26px] h-[26px] flex items-center justify-center rounded-[6px] border border-transparent transition-all duration-[220ms]',
