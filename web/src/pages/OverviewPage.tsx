...
export function OverviewPage() {
  ...
  const filtered = useMemo(() => { if (filter === 'running') return running; if (filter === 'stopped') return stopped; return containers }, [containers, filter, running, stopped])

  return (
    <div className="animate-pagein">
      <PageHeader title="Overview" sub="System-wide container health at a glance" actions={<><Button variant="ghost" size="sm" onClick={loadAll} disabled={state.loading}><RefreshCw className={cn('w-3 h-3', state.loading && 'animate-spin')} /> Refresh</Button><Button variant="primary" size="sm" onClick={() => setDeployOpen(true)}><Plus className="w-3 h-3" /> Deploy</Button></>} />
      ...
      <div className="mt-6 p-6 bg-[var(--bg-surface)] border border-[rgba(124,58,237,0.18)] rounded-[22px] flex items-center gap-[18px] relative overflow-hidden">
        ...
        <div className="flex-1"><div className="text-[14.5px] font-bold mb-1">Kubernetes support</div><div className="text-[12.5px] text-[var(--text-secondary)] leading-relaxed">Full cluster management, pod inspection, workload scaling, and AI-powered anomaly detection.</div>
          <div className="flex gap-1.5 flex-wrap mt-2">{['Pod Management','Deployments','Services & Ingress','RBAC','Helm Charts'].map(f => (<span key={f} className="text-[10.5px] font-medium px-2 py-1 rounded-[5px] bg-[rgba(167,139,250,0.07)] border border-[rgba(167,139,250,0.14)] text-[#c4b5fd]">{f}</span>))}</div>
        </div>
...
function ContainerCard({ container: c }: { container: Container }) {
  ...
  const isRunning = c.state === 'running'; const isPaused = c.state === 'paused'; const name = c.name.replace(/^\//, '')
  const act = async (action: 'start' | 'stop' | 'restart' | 'pause' | 'unpause') => { setActLoading(true); try { await api.containers[action](c.id); await loadAll(); toast(`Container ${action}ed`, 'success') } catch { toast(`Failed to ${action} container`, 'error') } finally { setActLoading(false) } }

  return (
    <Card className={cn('px-5 py-4 hover:border-[var(--border-bright)] hover:shadow-[var(--shadow-hover)]', 'grid grid-cols-[1fr_auto_auto_auto_auto] items-center gap-4')}>
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0"><div className="text-[13.5px] font-semibold text-[var(--text-primary)] truncate">{name}</div><div className="text-[11px] text-[var(--text-muted)] font-mono truncate">{c.image}</div></div>
      </div>
      <StatusBadge state={c.state} />
      <div className="text-[11.5px] text-[var(--text-muted)] font-mono hidden xl:block">{formatPorts(c.ports) || '\u2014'}</div>
      <div className="text-[11.5px] text-[var(--text-muted)] hidden lg:block">{formatUptime(c.created)} ago</div>
      <div className="flex items-center gap-1.5">
        {isPaused ? (<Button variant="success" size="xs" onClick={() => act('unpause')} disabled={actLoading}>\u25b6 Resume</Button>
        ) : isRunning ? (<><Button variant="ghost" size="xs" onClick={() => act('pause')} disabled={actLoading}>{'\u23f8'} Pause</Button><Button variant="ghost" size="xs" onClick={() => act('restart')} disabled={actLoading}>\u21ba Restart</Button><Button variant="danger" size="xs" onClick={() => act('stop')} disabled={actLoading}>\u25a0 Stop</Button></>
