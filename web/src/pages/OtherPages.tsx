          ...
          {pruneResult && (
            <div className="px-5 pb-4">
              <div className="rounded-[9px] bg-[rgba(16,217,138,0.06)] border border-[rgba(16,217,138,0.15)] p-3 text-[12px] text-[var(--text-secondary)] space-y-1">
                <div className="font-semibold text-[#10d98a] mb-1">✓ Prune complete — freed {formatBytes(pruneResult.reclaimed)}</div>
                {pruneResult.containers_deleted.length > 0 && <div>Containers removed: {pruneResult.containers_deleted.length}</div>}
                {pruneResult.images_deleted > 0        && <div>Images removed: {pruneResult.images_deleted}</div>}
                {pruneResult.volumes_deleted.length > 0 && <div>Volumes removed: {pruneResult.volumes_deleted.length}</div>}
                {pruneResult.networks_deleted.length > 0 && <div>Networks removed: {pruneResult.networks_deleted.length}</div>}
              </div>
            </div>
          )}
        </SettingsSection>
    ...
    <Card className="border-[rgba(124,58,237,0.15)] p-12 flex items-center gap-8 mt-2">
      ...
      <div>
        ...
        <div className="flex gap-2 flex-wrap">
          {features.map(f => (
            <span key={f} className="text-[11px] font-semibold px-3 py-1 rounded-full bg-[rgba(167,139,250,0.08)] border border-[rgba(167,139,250,0.18)] text-[#c4b5fd]">{f}</span>
          ))}
        </div>
      </div>
    </Card>
...
export function K8sOverviewPage() {
  return <div className="animate-pagein"><PageHeader title="Kubernetes Overview" sub="Cluster health at a glance" />
    <K8sPlaceholder title="Cluster Overview" icon="⎈" features={['Node status','Resource quotas','Namespace summary','Event stream','Workload health']} /></div>
}
export function K8sPodsPage() {
  return <div className="animate-pagein"><PageHeader title="Pods" sub="Running and pending workloads" />
    <K8sPlaceholder title="Pod Management" icon="📦" features={['Pod listing','Log streaming','Shell exec','Resource limits','Restart policy']} /></div>
}
export function K8sDeploymentsPage() {
  return <div className="animate-pagein"><PageHeader title="Deployments" sub="Workload deployments and rollouts" />
    <K8sPlaceholder title="Deployments" icon="🚀" features={['Rolling updates','Rollback','Scale replicas','Strategy config','History']} /></div>
}
export function K8sServicesPage() {
  return <div className="animate-pagein"><PageHeader title="Services" sub="Kubernetes services and ingress" />
    <K8sPlaceholder title="Services & Ingress" icon="🌐" features={['ClusterIP','NodePort','LoadBalancer','Ingress rules','DNS resolution']} /></div>
}
export function K8sNamespacesPage() {
  return <div className="animate-pagein"><PageHeader title="Namespaces" sub="Kubernetes namespace management" />
    <K8sPlaceholder title="Namespaces" icon="📁" features={['Namespace CRUD','Resource quotas','RBAC','Context switching','Label management']} /></div>
}
