...
export function NetworksPage() {
  ...
  const deleteNetwork = async (id: string, name: string) => {
    if (['bridge','host','none'].includes(name)) { toast('Cannot delete default networks', 'error'); return }
    if (!window.confirm(`Delete network "${name}"?`)) return
    try { await api.networks.delete(id); await loadAll(); toast('Network deleted', 'success') }
    catch { toast('Delete failed — network may have active endpoints', 'error') }
  }
  ...
  const createNetwork = async () => {
    ...
    setCreating(true)
    try {
      await api.networks.create(netName.trim(), netDriver); await loadAll()
      toast(`Network "${netName}" created`, 'success')
      setCreateOpen(false); setNetName('')
    } catch { toast('Create failed', 'error') }
    finally { setCreating(false) }
  }

  function driverVariant(driver: string): 'cyan' | 'running' | 'purple' | 'default' {
    if (driver === 'bridge')  return 'cyan'
    if (driver === 'host')    return 'running'
    if (driver === 'overlay') return 'purple'
    return 'default'
  }
    ...
    <div className="animate-pagein">
      ...
      <Card className="overflow-hidden">
        <Table>
          ...
          <TableBody>
            {networks.length === 0 ? (
              <EmptyTableRow cols={7} icon="🌐" title={state.loading ? 'Loading networks…' : 'No networks'} />
            ) : (
              networks.map(n => {
                  ...
                  <TableRow key={n.id}>
                    <TableCell>
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <div className="font-medium text-[13px] text-[var(--text-primary)]">{n.name}</div>
                          <div className="font-mono text-[10.5px] text-[var(--text-muted)]">{n.id.slice(0,12)}</div>
                        </div>
                      </div>
                    </TableCell>
                    <TableCell>
                      <Badge variant={driverVariant(n.driver)} className="font-mono">{n.driver}</Badge>
                    </TableCell>
                    <TableCell>{n.scope}</TableCell>
                    <TableCell><span className="font-mono text-[11.5px]">{subnet}</span></TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <button onClick={() => deleteNetwork(n.id, n.name)} disabled={isDefault}
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input id="netName" value={netName} onChange={e => setNetName(e.target.value)} placeholder="my-network" autoFocus />
            </div>
            <div>
              <Label>Driver</Label>
