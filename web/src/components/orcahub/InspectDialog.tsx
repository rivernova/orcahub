...
export function InspectDialog({ open, onClose, containerId, containerName }: InspectDialogProps) {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setData(null); setTop(null)
    Promise.all([
      api.containers.inspect(containerId),
      api.containers.top(containerId).catch(() => null),
    ]).then(([inspect, topResult]) => {
      setData(inspect)
      setTop(topResult)
      ...
      <DialogContent className="max-w-2xl w-full">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {data && (
              <Badge variant={data.state === 'running' ? 'running' : 'exited'} dot>
                {data.state}
              </Badge>
            )}
          </DialogTitle>
        ...
        {loading && (
          <div className="flex items-center justify-center h-48 text-[var(--text-muted)] text-[13px]">
            Loading…
          </div>
        )}
        ...
        {!loading && data && (
          <Tabs value={tab} onValueChange={setTab}>
            ...
            <TabsContent value="overview" className="space-y-3 max-h-[55vh] overflow-y-auto">
              ...
              {data.finished_at && data.state !== 'running' && (
                <Row label="Finished" value={new Date(data.finished_at).toLocaleString()} />
              )}
              {data.exit_code !== 0 && <Row label="Exit code" value={String(data.exit_code)} />}
              {data.cmd?.length > 0 && (
                <Row label="Command" value={data.cmd.join(' ')} mono />
              )}
              {data.entrypoint?.length > 0 && (
                <Row label="Entrypoint" value={data.entrypoint.join(' ')} mono />
              )}
              {data.ports?.length > 0 && (
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex gap-1.5 flex-wrap">
                    {data.ports.map((p, i) => (
                      <Badge key={i} variant="cyan" className="font-mono text-[10.5px]">
              ...
              {data.labels && Object.keys(data.labels).length > 0 && (
                <div>
                  <div className="text-[11.5px] text-[var(--text-muted)] mb-1.5">Labels</div>
                  <div className="bg-[var(--bg-raised)] border border-[var(--border)] rounded-[9px] p-3 space-y-1 max-h-32 overflow-y-auto">
              ...
              {data.env?.length > 0 ? (
                <div className="bg-[var(--bg-raised)] border border-[var(--border)] rounded-[9px] p-3 space-y-1">
                  {data.env.map((e, i) => {
                    ...
                    const val = rest.join('=')
                    return (
                      <div key={i} className="flex gap-2 text-[11.5px] font-mono">
                        <span className="text-[#00d4ff] flex-shrink-0 min-w-[120px]">{key}</span>
              ...
              {data.mounts?.length > 0 ? data.mounts.map((m, i) => (
                <div key={i} className="bg-[var(--bg-raised)] border border-[var(--border)] rounded-[9px] p-3 text-[12px]">
                  <div className="flex items-center gap-2 mb-2">
                    <Badge variant={m.type === 'volume' ? 'cyan' : 'default'} className="text-[10px]">{m.type}</Badge>
                    <Badge variant={m.rw ? 'running' : 'default'} className="text-[10px]">{m.rw ? 'rw' : 'ro'}</Badge>
                  </div>
                  <div className="font-mono space-y-0.5">
                    <div><span className="text-[var(--text-muted)]">src: </span><span className="text-[var(--text-secondary)] break-all">{m.source}</span></div>
                    <div><span className="text-[var(--text-muted)]">dst: </span><span className="text-[var(--text-secondary)] break-all">{m.destination}</span></div>
                  </div>
                </div>
              )) : (
              ...
              {data.networks && Object.keys(data.networks).length > 0 ? (
                Object.entries(data.networks).map(([netName, ep]) => (
                  <div key={netName} className="bg-[var(--bg-raised)] border border-[var(--border)] rounded-[9px] p-3 text-[12px] font-mono">
                    ...
                    <Row label="IP"          value={ep.ip_address}  mono small />
                    <Row label="Gateway"     value={ep.gateway}     mono small />
                    <Row label="MAC"         value={ep.mac_address} mono small />
                    <Row label="Network ID"  value={shortId(ep.network_id)} mono small />
                  </div>
                ))
              ) : (
                  ...
                  <table className="w-full text-[11.5px]">
                    <thead>
                      <tr className="border-b border-[var(--border)]">
                        {top.titles.map((t, i) => (
                          <th key={i} className="text-left px-2 py-1.5 text-[var(--text-muted)] font-semibold tracking-wide">{t}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {top.processes.map((row, i) => (
                        <tr key={i} className={cn('border-b border-[var(--border)]', i % 2 === 0 && 'bg-[var(--bg-glass)]')}>
                          {row.map((cell, j) => (
                            <td key={j} className="px-2 py-1.5 font-mono text-[var(--text-secondary)] whitespace-nowrap">{cell}</td>
                          ))}
                        </tr>
                      ))}
                    </tbody>
...
function Row({ label, value, mono = false, small = false }: {
  ...
  return (
    <div className={cn('flex items-start gap-3', small && 'gap-2')}>
      <span className={cn('text-[var(--text-muted)] flex-shrink-0', small ? 'text-[10.5px] w-20' : 'text-[11.5px] w-28')}>
        {label}
      </span>
      <span className={cn(
        'break-all text-[var(--text-secondary)]',
